...
public class CadFonesSw extends JFrame {
	...
	private JButton btLimpa = null;
	private JButton btSair = null;

	private List<Fone> fones = new ArrayList<Fone>(); // @jve:decl-index=0:
	private TableModel foneModel = new FoneTable(fones);
	...
	private JTextField getTfDdd() {
		if (tfDdd == null) {
			try {
				...
				System.err.println("DDD: máscara inválida!");
	...
	private JTable getTbFones() {
		if (tbFones == null) {
			...
			tbFones.setAutoCreateRowSorter(true);

			TableColumn cm = tbFones.getColumnModel().getColumn(0);
			cm.setCellEditor(new DefaultCellEditor(new JComboBox<TipoFone>(TipoFone.values())));
		...
		if (btInclui == null) {
			...
			btInclui.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					fones.add(fone);
					tbFones.tableChanged(new TableModelEvent(foneModel));
	...
	/**
	 * This method initializes btLimpa
	 * 
	 * @return javax.swing.JButton
	 */
	private JButton getBtLimpa() {
		if (btLimpa == null) {
			btLimpa = new JButton();
			btLimpa.setBounds(new Rectangle(195, 105, 94, 31));
			btLimpa.setMnemonic(KeyEvent.VK_L);
			btLimpa.setText("Limpar");
			btLimpa.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					if (JOptionPane.showConfirmDialog(null,
							"Confirma a exclusão de todos os telefones da lista?") == 0) {
						fones.clear();
						tbFones.tableChanged(new TableModelEvent(foneModel));
					}
				}
	...
	private JPanel getJContentPane() {
		if (jContentPane == null) {
			...
			jContentPane.add(getBtLimpa(), null);
