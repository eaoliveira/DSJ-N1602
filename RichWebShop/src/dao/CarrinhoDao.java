...
@Stateless
public class CarrinhoDao implements TimerInterface {
	...
	public void avisa(Timer timer) {
		...
		now.add(Calendar.MINUTE, -2);
			try {
				Query consulta = session.createQuery("from Carrinho where data <= :data");
				consulta.setParameter("data", now.getTime());
				List<Carrinho> lista = (List<Carrinho>) consulta.getResultList();

				System.out.println(now.getTime() + " Carrinhos:" + lista.size());
