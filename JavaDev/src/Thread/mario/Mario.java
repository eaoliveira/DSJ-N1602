...
public class Mario {
	...
	public Mario(JFrame frame, int pos) {
		...
		BufferedImage[] imgs = splitImages(createTransparentImage("mario2.png",
				null), 6, 4);
	...
	public void draw(Graphics2D g) {
		...
		if(direita) 
			...
			img = new Rectangle2D.Double(sprite.getX() + sprite.getWidth() - 11, sprite.getY(), 11, sprite.getHeight() + 1);
	...
	public BufferedImage[] splitImages(BufferedImage image, int col, int row) {
		...
		for (int y = 0; y < row; y++) {
			for (int x = 0; x < col; x++) {
				...
				g.drawImage(image, 0, 0, w, h, x * w, y * h, (x + 1) * w, (y + 1) * h,
						null);
				g.dispose();
				num++;
			}
	...
	public static BufferedImage createTransparentImage(String img, Color fundo) {
		...
		Image image = new ImageIcon(Mario.class.getClassLoader().getResource(
				imgName)).getImage();
