...
public class SerialConnection implements SerialPortEventListener,
   ...
   * @param messageAreaOut The TextArea that messages that are to be sent out of
   *        the serial port are entered into.
   ...
   * the port to a closed state, throws a <code>SerialConnectionException</code>,
   * and returns. Gives a timeout of 30 seconds on the portOpen to allow other
   * applications to reliquish the port if have it open and no longer need it.
   */
  ...
  public void setConnectionParameters() throws SerialConnectionException {
    ...
    try {
      sPort.setSerialPortParams(parameters.getBaudRate(), parameters
        .getDatabits(), parameters.getStopbits(), parameters.getParity());
    }
    catch(UnsupportedCommOperationException e) {
  ...
  /**
   * Handles ownership events. If a PORT_OWNERSHIP_REQUESTED event is received a
   * dialog box is created asking the user if they are willing to give up the
   ...
   * messageAreaOut. When a <code>KeyEvent</code> occurs the <code>char</code>
   * that is generated by the event is read, converted to an <code>int</code>
   * and writen to the <code>OutputStream</code> for the port.
   */
