...
public class TelaChat2 extends JFrame implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (botao.equals(btConectar)) {
			...
			recebeMensagem = new RecebeMensagem(nome);
			ativarBotao(true);
	...
	class RecebeMensagem implements Runnable {
		...
		public RecebeMensagem(String nome) {
			this.nome = nome;
			try {
		...
		public void enviar(String txt) {
			if (enviaMensagem != null) {
				enviaMensagem.println(nome+"? "+txt);
				enviaMensagem.flush();
			} else {
