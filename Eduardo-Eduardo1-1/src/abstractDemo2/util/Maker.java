...
public class Maker {
  ...
  public static JMenu makeMenu(MenuDefs menu, MenuDefs[] itens,
    ...
    if(menu.atalho != -1)
      mn.setMnemonic(menu.atalho);
    mkMenu(mn, itens, ac);
    return mn;
  }

  public static JPopupMenu makePopupMenu(MenuDefs[] itens, ActionListener ac) {
    JPopupMenu mn = new JPopupMenu();
    mkMenu(mn, itens, ac);
    return mn;
  }

  private static void mkMenu(JComponent mn, MenuDefs[] itens, ActionListener ac) {
    if(itens != null) {
      for(MenuDefs menuIt : itens) {
        if(menuIt.item == null)
          ...
          if(menuIt.comando != null)
            it.setActionCommand(menuIt.comando);
...
public static JToolBar makeToolBar(MenuDefs[] itens, ActionListener ac) {
  ...
  public static Border mkTitledBorder(String titulo) {
    return BorderFactory.createCompoundBorder(BorderFactory.createTitledBorder(
      BorderFactory.createEtchedBorder(), titulo), BorderFactory
